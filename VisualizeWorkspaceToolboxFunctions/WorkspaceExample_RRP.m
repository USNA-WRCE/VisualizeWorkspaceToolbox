...
% Draw the workspace and dexterous workspace for the RRP arm
%
...
% H_e2o = Rz(q(1))Tx(L(1))Rz(q(2))Tx(L(2))Tx(q(3));
...
kinematics = sprintf('Rz[%d,%d]Tx%dRz[%d,%d]Tx%dTx[%d,%d]',...
    rad2deg(q_lim(1,:)),...
    L(1),...
    rad2deg(q_lim(2,:)),...
    L(2),...
...
ylabel(axs,'y (cm)');
grid(axs,'on');
xlim(axs,[-(sum(L)+q_lim(3,2)),(sum(L)+q_lim(3,2))]+[-1,1]);
ylim(axs,[-(L(2)+q_lim(3,2)),(sum(L)+q_lim(3,2))]+[-1,1]);
